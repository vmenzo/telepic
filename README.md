...
上传图片可以直接把图片发送给 Bot。需要切换本地存储或对象存储时，进入 Bot 的“存储控制台”选择“上传到本地”或“上传到存储桶”；改名、标签、删除、加入相册、生成链接格式等管理操作也在控制台按钮中完成。
...
- Web 上传入口支持为本次上传选择“跟随当前配置 / 本地存储 / 对象存储”。
- Telegram 控制台支持为当前 Telegram 用户选择默认上传位置。
...
curl -H "Authorization: Bearer YOUR_TOKEN" \
  -H "x-storage-driver: local" \
  -F "image=@photo.png" \
  http://127.0.0.1:8787/api/upload
```

`x-storage-driver` 可选，支持 `local` 或 `s3`；不传时跟随当前存储配置。
...
curl -X POST \
  ...
  -d '{"url":"https://example.com/image.png","storageDriver":"s3"}' \
