...
Telepic 是一个可自托管的开源图床，目标是同时服务 Web 管理、API 上传和 Telegram Bot 管理。当前版本默认使用 SQLite 保存元数据，不依赖额外数据库服务；也可以切回 JSON 文件模式。图片可存本地或切换到 S3 兼容对象存储。
...
- 数据库：默认 SQLite，支持从旧版 `data/db.json` 首次启动时自动导入，也可通过环境变量切回 JSON。
- 对象存储：支持 S3 兼容接口，可对接 AWS S3、Cloudflare R2、Backblaze B2 S3、MinIO。
- Linux 部署：支持常见发行版的自动安装脚本，包括 Debian/Ubuntu、CentOS/RHEL/Rocky、Fedora、Alpine、Arch Linux。
...
如果你 fork 了项目，可以指定自己的仓库：

```bash
curl -fsSL https://raw.githubusercontent.com/vmenzo/telepic/main/scripts/install.sh \
  | sudo env TELEPIC_REPO=https://github.com/yourname/telepic.git PUBLIC_URL=https://img.example.com sh
```
...
首次从旧版 JSON 升级到 SQLite 时，如果 SQLite 数据库为空，Telepic 会自动读取 `data/db.json` 并导入图片、token 和事件记录。想继续使用纯 JSON 文件也可以：
...
| `DATA_DIR` | 数据和图片保存目录。 |
| `DATABASE_DRIVER` | 数据库驱动，默认 `sqlite`，也可设为 `json`。 |
| `DATABASE_FILE` | SQLite 数据库文件路径，默认 `./data/telepic.sqlite`。 |
